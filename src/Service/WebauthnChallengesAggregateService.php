...
namespace BlackCat\Database\Packages\WebauthnChallenges\Service;
 ...
 */
final class WebauthnChallengesAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private WebauthnChallengeRepository $webauthnChallengeRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'webauthn_challenges' => $this->webauthnChallengeRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\WebauthnChallenges\ModuleException("Record {$idStr} not found for locking.");
            }
